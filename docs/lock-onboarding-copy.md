# 2. Lock Onboarding (humanized copy)
...
!!! info "Two of the eight steps below wait on Kafka, not on a REST response"
    ...
    **Step 1, Choose a property** and **step 4, Customise the lock** both end
    with the app waiting. It is not waiting on a slow Spintly call. It is
    waiting on a **Kafka message** coming back from Spintly, which creates the
...
screens the user sees, then the diagram of what is running behind them. Any
screen can be clicked to open it full size.
...
<figcaption><strong>Chosen</strong>Step 4 hands the <code>siteId</code> behind this row to <code>addLock</code>, and that <code>siteId</code> has to be <code>synced</code> before the lock will be accepted.</figcaption>
</figure>
<figure>
...
answer back either way, so it never finds out which one it was. Binaryveda's
backend chooses between them.
...
So this step is four instruction cards, swiped through in order, and a **Scan**
button. No SDK, no Binaryveda's backend and no Spintly.
...
<figcaption><strong>1 of 4, open the door</strong>Asked first because setup writes to the lock. A door left latched while that is happening can shut someone out of the room they are standing outside.</figcaption>
</figure>
<figure>
...
<figcaption><strong>2 of 4, take the back panel off</strong>The panel on the inside face of the door. The button in the next card is underneath it.</figcaption>
</figure>
<figure>
...
<figcaption><strong>3 of 4, hold R for 3 seconds</strong>The one action in this whole flow that happens on the hardware rather than in the app, and nothing after it works without it. A factory fresh lock ignores a BLE scan until this is done.</figcaption>
</figure>
<figure>
...
<figcaption><strong>4 of 4, wait for the sound cue, then scan</strong>The cue tells the user the lock is listening. This Scan button is the first thing on the page to reach the Config SDK, and it starts step 3.</figcaption>
</figure>
</div>
...
**The Kafka consumer writes what the app is polling for.** Android's
...
platform shows an error. Both keep asking.
...
else. [My Access](lock-settings.md#2-my-access), a user's detail screen in
[User Management](user-management.md), and an invited user setting up their own
access all get what a Config SDK call needs that way, without carrying the ids
across from here.

Whichever of the two runs, it has to run after the access point exists, because
it names the access point. The Kafka message provides that ordering: Spintly
does not publish `access_point_create` until the access point is there.
...
!!! note "Resuming an interrupted onboarding"
    ...
    **`resumeOnboarding` is a state machine over those same synced flags**, and
    ...
    point is synced. Repeating the query walks it forward as each message
    lands.
...
<figcaption><strong>Onboarding pending</strong>No tiles and no Unlock button, because none of it works yet. Resume Onboarding runs the query in the note above rather than starting the flow again from the top, so the half finished lock is picked up where it stopped.</figcaption>
</figure>
</div>
...
<figcaption><strong>Pushed</strong>Continue does more than move on. Both platforms re-enter the check and read the two versions again, so a push that did not take is caught rather than assumed.</figcaption>
</figure>
</div>
...
<figcaption><strong>Behind &ldquo;Where to find the passcode?&rdquo;</strong>The factory passcode is printed in the manual, not held anywhere in the app or on the backend, so the user has to type it rather than the app filling it in.</figcaption>
</figure>
</div>
...
<figcaption><strong>Both done</strong>Finish ends onboarding whether either row was used or not, so neither is required. The tick is drawn from what the lock reports, not from anything held on Binaryveda's backend.</figcaption>
</figure>
</div>
...
<figcaption><strong>Enrolled</strong>The template is on the lock and the session is closed. Add another fingerprint runs the whole cycle again, so one lock can hold several fingers.</figcaption>
</figure>
<figure>
...
Four screens. The middle two are the app redrawing on an `NFCProcessState` the
SDK has just reported, one callback at a time, so the user can see where the
enrolment has got to.
...
<figcaption><strong>Before anything connects</strong>The whole enrolment runs over BLE to the lock, so the phone has to stay near it throughout. Next calls <code>scanAndConnectCardDevice</code>.</figcaption>
</figure>
<figure>
...
<figcaption><strong>Enrolled</strong>The assign succeeded and the session is closed. Add another RFID Card runs the whole thing again, so a lock can hold more than one card.</figcaption>
</figure>
</div>
...
<figcaption><strong>The lock, onboarded</strong>The card is drawn from the name, area and image set in step 4, and the model came out of the catalogue lookup in step 3. Status BLE means the phone is reaching the lock directly rather than through a gateway. <a href="home.md">Home</a> covers what keeps this card live.</figcaption>
</figure>
</div>
