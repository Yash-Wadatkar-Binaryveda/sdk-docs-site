...
locks with them. [User Onboarding](user-onboarding.md) runs once at the start,
while this runs whenever an invite arrives.
...
The platforms differ only in the last argument and what comes back through it.
iOS takes a `completion` closure, and Android takes a `callback` and reports
through `SpintlyCompletionCallback<Void>`.
